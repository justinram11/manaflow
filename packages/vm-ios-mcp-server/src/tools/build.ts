...
const iosBuild: ToolHandler = async (params, allocationId) => {
  ...
  return buildLimit(async () => {
    ...
    console.log(`[ios-build] ${cmd}`);

    try {
      const output = exec(cmd, { timeout: 30 * 60 * 1000 });
...
const iosBuildAndRun: ToolHandler = async (params, allocationId) => {
  ...
  return buildLimit(async () => {
    ...
    const cmd = `cd "${projectDir}" && ${args.join(" ")}`;
    console.log(`[ios-build-and-run] ${cmd}`);
    ...
    try {
      ...
      if (appPath) {
        ...
        try {
          ...
          exec(`xcrun simctl launch "${alloc.simulatorUdid}" "${bundleId}"`);
...
const iosClean: ToolHandler = async (params, allocationId) => {
  ...
  try {
    const output = exec(`cd "${projectDir}" && ${args.join(" ")}`);
    return { success: true, output };
  } catch (error) {
