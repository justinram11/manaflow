...
const PORT = Number(process.env.CMUX_VM_MCP_PORT) || 4860;
      ...
      case "initialize": {
        return {
          ...
          result: {
            ...
            capabilities: { tools: {} },
            serverInfo: {
              name: "cmux-vm-android-mcp-server",
              version: "1.0.0",
            },
          },
          id,
...
async function handleToolCall(request: Request, allocationId: string): Promise<Response> {
  ...
  try {
    ...
    console.error(`[android-mcp] tool ${toolName} failed:`, error);
    return jsonResponse(
...
async function handleScreenshot(request: Request, allocationId: string): Promise<Response> {
  ...
  const screenshotTool = getTool("android_screenshot");
  if (!screenshotTool) {
    return jsonResponse({ error: "android_screenshot tool unavailable" }, 500);
  }
  ...
  try {
    ...
    console.error("[android-mcp] screenshot failed:", error);
    return jsonResponse(
...
console.log(`[vm-android-mcp-server] listening on 0.0.0.0:${PORT}`);
